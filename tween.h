...
/*
	template <class A> static int addRest(Tween &t, const A &head)
	{
		t.add(head);
	}

	template <class... B> static int addRest(Tween &t, const B& ... tail)
	{
		addRest(t, tail...);
	}

	template <class... B> static int to(float totalTime, const B& ... tail)
	{
		Tween t;
