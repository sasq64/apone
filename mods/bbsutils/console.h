...
public:
    ...
    */
    ...
    utils::vec<int, 2> getSize() const {
        return utils::vec<int, 2>(width, height);
    }
    int getCursorX() const { return curX; }
    int getCursorY() const { return curY; }

    utils::vec<int, 2> getCursor() const {
        return utils::vec<int, 2>(curX, curY);
    }
    void moveCursor(const utils::vec<int, 2>& pos) { moveCursor(pos.x, pos.y); }
    void crlf() { moveCursor(0, curY++); }

    virtual void showCursor(bool show) { impl_showcursor(show); }
